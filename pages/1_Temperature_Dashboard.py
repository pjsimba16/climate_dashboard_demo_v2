...
def percentile_series_cached(df_dates: pd.Series, df_values: pd.Series, pct: int) -> pd.DataFrame:
    """Returns a DF with columns ['date','p'] for the requested percentile by month."""
...
with top_l:
    if st.button("← Home", help="Back to Home map"):
        ...
        try:
            ...
            st.rerun()
st.markdown(f"### Temperature - {title_country}")

# --------------- Precompute lists ----------
countries_iso = sorted(COUNTRY_TEMP["Country"].dropna().astype(str).str.upper().unique().tolist())
country_options = ["—"] + countries_iso

if "opt_iso3" not in st.session_state:
    st.session_state["opt_iso3"] = iso3_q if iso3_q in country_options else "—"
...
with lc:
    ...
    # ---------- Map ----------
    if iso3 and iso3 != "—":
        # Build latest-by-ADM1 dataframe for choropleth
    ...
    if indicator == "Precipitation":
        st.query_params.update({"page": "2_Precipitation_Dashboard"})
...
with rc:
    ...
    with st.spinner("Computing mini trend…"):
        s_scope = df_scope[["date","avg"]].dropna().copy()
        last12 = s_scope.tail(12)
        if not last12.empty:
            fig_sp = go.Figure()
            fig_sp.add_trace(go.Scatter(x=last12["date"], y=last12["avg"], mode="lines+markers",
    ...
    with k1:
        st.metric("Latest Avg Temp", f"{latest:.2f} °C" if np.isfinite(latest) else "—",
                  help="Most recent monthly average temperature in the selected range/area.")
    with k2:
    ...
    with st.spinner("Rendering time-series charts…"):
        ...
        st.plotly_chart(chart_with_ribbon("Diurnal Temperature Range (placeholder)", series, "orange"),
                        use_container_width=True)
        st.plotly_chart(chart_with_ribbon("Maximum Temperature (placeholder)", series, "verm"),
                        use_container_width=True)
        st.plotly_chart(chart_with_ribbon("Minimum Temperature (placeholder)", series, "navy"),
                        use_container_width=True)
    ...
    def _percentile_block():
        ...
        with st.spinner("Updating percentile charts…"):
            percentile_chart("Average Temperature", series, "green")
            percentile_chart("Maximum Temperature (placeholder)", series, "pink")
            percentile_chart("Minimum Temperature (placeholder)", series, "sky")
